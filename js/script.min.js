...
$('#burger').on('click',function(){$('.fullscreen-nav-container').slideToggle(300);});$('.fullscreen-nav-holder a, .turn-home').on('click',function(){$('#burger').trigger('click');});if(window.location.pathname.includes('/blog/')){const blogMenuItem=jQuery.grep($('.menu-item'),(ele,i)=>ele.textContent.trim().toLowerCase()==='blog');if(blogMenuItem.length)blogMenuItem[0].classList.add('active');}else{$(window).on('scroll',function(){var counter=0;$('section').each(function(){if($(window).scrollTop()+80>=$(this).offset().top){$('.menu-item').removeClass('active');$('.menu-item.'+counter).addClass('active');$('.mobile-menu-item').removeClass('active');$('.mobile-menu-item.'+counter).addClass('active');}
++counter;});});}
function toggleLogoImg(id){var img=document.getElementById(id).src;if(img.indexOf('logo-dark.svg')!=-1){document.getElementById(id).src='/images/logo.svg';}else{document.getElementById(id).src='/images/logo-dark.svg';}}
